...
class Pedal(nn.Module):
    ...
    def forward(self, feature, centers, position,epoch):
        ss=self.k+int(20*epoch/60)
        ...
        for p in range(feature.size(0)):
            ...
            #print(dist_map.size(),position.size())
           ...
           # print(neg.size(),'...................')
            ...
            x = ((-1 * self.scale * neg[:, :ss]).exp().sum(dim=1)).log()
...
class Emdal(nn.Module):

    def __init__(self, scale=10, k=5):
        super(Emdal, self).__init__()
        self.scale =scale
        self.k = k


    def forward(self, emfeature, centers, position, epoch):
        #ss=self.k+int(20*epoch/60)
        #a = np.array(range(1,feature.size(1)+1))
        #position = torch.from_numpy(a).cuda()
        feature = emfeature[:,position,:]
        loss = 0
        for p in range(feature.size(0)):
            part_feat = feature[p, :, :]
            part_centers = centers[p, :, :]
            k, l = part_feat.size(0), part_centers.size(0)
            dist_map2 = part_feat.pow(2).sum(dim=1, keepdim=True).expand(k, l) + \
                       part_centers.pow(2).sum(dim=1, keepdim=True).expand(l, k).t()
            dist_map2.addmm_(1, -2, part_feat, part_centers.t())
           # print(dist_map2.size(),position.size(),position)
            trick2 = torch.arange(dist_map2.size(1)).cuda().expand_as(dist_map2)
           # print(trick2.size())
            neg2, _ = dist_map2[trick2!=position.unsqueeze(dim=1).expand_as(dist_map2)].view(dist_map2.size(0), -1).sort(dim=1)

            x = ((-1 * self.scale * neg2[:, :k]).exp().sum(dim=1)).log()
            y = ((-1 * self.scale * neg2).exp().sum(dim=1)).log()
