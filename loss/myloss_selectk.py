...
class Pedal(nn.Module):
    ...
    def __init__(self, scale=10, tau=0.5, lambda0=1,delta = 1):
    ...
    def forward(self, feature, centers, position):
        ...
        for p in range(feature.size(0)):
            ...
            ks = self.adaptive_selection(dist_map,centers)
            k = ks.numpy()+5
            print(k,type(k),int(k[p]),dist_map)
            ...
            x = ((-1 * self.scale * neg[:, :int(k[p])]).exp().sum(dim=1)).log()
    ...
    def adaptive_selection(self, inputs, targets):
        ###minimize distances between similar person images
        ...
        ###maximize distances between all person images
